...
"""
    ...
    @gl.public.write
    def execute_ruling(self, in_favour_of: str = "") -> None:
        """
        Owner executes the final resolution of a dispute.

        The settlement winner is bound directly to the stored consensus ruling
        (self.dispute_ruling) generated by GenLayer AI consensus, rather than
        an owner-supplied winner.
        ...
        # Fallback to parameter only if stored consensus ruling could not be parsed
        if ruling not in ("BUYER", "SELLER") and in_favour_of:
            ruling = in_favour_of.strip().upper()
